...
这篇文章不是一个脱离项目的模型练习。当时我在做智慧农业智能车，前期主代码和硬编码路径也是我负责。路径做完以后，真正值得单独沉淀的技术部分，是我完成的授粉YOLO模型。后面的实车数据采集、清洗、标注、训练和部署，都是为比赛里的授粉任务服务。路径和模型完成以后，我不再持续参与整车的全部调试，后期主要在摄像头出问题时过去处理。这篇只讲我实际做过、也能说明白的视觉部分，不拿整车结果代替技术细节。

> **目前的证据边界：** 智慧农业项目最终获得全国总决赛一等奖，但这是团队系统的结果，不能直接当成模型指标。现在还能核对的是第一批 2600 张图片的标注过程、训练与验证脚本、两份数据配置，以及我对采集、清洗、训练和部署流程的记录；最终数据规模、类别分布、训练指标、车端推理速度和比赛现场原始材料暂时没有找全，所以这篇不补这些数字。
